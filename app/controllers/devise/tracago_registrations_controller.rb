class Devise::TracagoRegistrationsController < Devise::RegistrationsController
  def create
  ...
  def _signup_path(*stuff)
    return itps_signup_path(*stuff) if _itps_origin?
    return signup_path(*stuff)
  end
  ...
  def _user_path(*stuff)
    return itps_user_path(user) if _itps_origin?
    return user_path(user)
  end

  def _get_out!(user)
    return redirect_to _signup_path(_back_params) if _user_invalid?
    return redirect_to _return_path if _user_valid? && _return_path.present?
    return redirect_to _user_path user if _user_valid?
  end
